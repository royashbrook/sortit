...
function face(cx: number, cy: number, s = 1): string {
  const ex = 5.5 * s
  const ey = 1.5 * s
  const er = 1.9 * s
  const sw = 2.4 * s
    ...
    },
    {
      ...
      svg: `<g fill="#F76B15" ${S}>` +
        [0, 45, 90, 135, 180, 225, 270, 315].map(a => {
          ...
          const p = (r: number, w: number) => `${Math.round(32 + c * r - s * w)},${Math.round(32 + s * r + c * w)}`
          const q = (r: number, w: number) => `${Math.round(32 + c * r + s * w)},${Math.round(32 + s * r - c * w)}`
