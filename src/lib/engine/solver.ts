...
import type { Move, OptimalResult, SolveOptions, SolveResult, Tube, Tubes } from './types.ts'

export function isComplete(tube: Tube, capacity: number): boolean {
  if (tube.length !== capacity) return false
  for (let i = 1; i < tube.length; i++) if (tube[i] !== tube[0]) return false
  return true
}

export function isWin(tubes: Tubes, capacity: number): boolean {
...
export function legalMoves(tubes: Tubes, capacity: number): Move[] {
  const moves: Move[] = []
...
export function applyMove(tubes: Tubes, move: Move): Tubes {
...
function ordered(tubes: Tubes, capacity: number): Move[] {
...
export function optimal(tubes: Tubes, capacity: number, { maxNodes = 400000 }: Pick<SolveOptions, 'maxNodes'> = {}): OptimalResult {
  ...
  const floor = (state: Tubes): number => {
    let r = 0
  ...
  function search(state: Tubes, g: number, bound: number): number | true {
...
export function solve(tubes: Tubes, capacity: number, { maxNodes = 200000, maxDepth = 300 }: SolveOptions = {}): SolveResult {
  const visited = new Set<string>()
  const path: Move[] = []
  let nodes = 0
  let aborted = false

  function search(state: Tubes, depth: number): boolean {
    if (isWin(state, capacity)) return true
  ...
  return solved
    ? { solved, moves: path.slice(), nodes, aborted }
    : { solved, moves: null, nodes, aborted }
}
