import type { Random, ShareResult } from './types.ts'
...
export function isDaily(seed: number): boolean {
  return seed === dailySeed()
}

export function seedUrl(seed: number): string {
...
export async function shareSeed(seed: number, title: string): Promise<ShareResult> {
