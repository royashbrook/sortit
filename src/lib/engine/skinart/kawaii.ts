...
function face(cx: number, cy: number, spread = 8, mouth: 'w' | 'smile' | 'o' | 'cat' = 'w'): string {
  const mouths = {
...
const body = (d: string, fill: string): string => `<path d="${d}" fill="${fill}" stroke="${INK}" stroke-width="${SW}" stroke-linejoin="round"/>`
const blob = (cx: number, cy: number, rx: number, ry: number, fill: string): string => `<ellipse cx="${cx}" cy="${cy}" rx="${rx}" ry="${ry}" fill="${fill}" stroke="${INK}" stroke-width="${SW}"/>`
