...
import { renderNut } from './nut-geometry.ts'
import type { SkinArt } from '../types.ts'
export { nutGeometry } from './nut-geometry.ts'
...
type Nut = [key: string, color: string, lit: string, shade: string, mark: keyof typeof MARKS | null]
const NUTS: Nut[] = [
...
export function nutArt(key: string, turn = 0, postTip: number | null = -1000, id?: string): string {
  const [name, color, lit, shade, mark]: Nut = NUTS.find(nut => nut[0] === key) ?? ['hid', '#929EA8', '#D7E0E6', '#4C5963', null]
