...
export function boltArt(seat: number, id: string): string {
  const threads = Array.from({ length: Math.ceil((seat - 5) / 5) }, (_, i) => {
    const y = 5 + i * 5
  ...
  return `<defs>
    ...
    <clipPath id="${id}-shaft"><path d="M22 5Q32 -1 42 5V${seat}Q32 ${seat + 5} 22 ${seat}Z"/></clipPath>
  </defs>
