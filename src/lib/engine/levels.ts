...
import { rng, shuffle } from './seed.ts'
import { solve, isComplete } from './solver.ts'
import type { BoardParams, LevelBoard, SeedBoard, Tubes } from './types.ts'
...
const FIRST_SALT: Partial<Record<number, number>> = { 2: 3 }
...
export function paramsFor(n: number): BoardParams {
...
function levelSeed(n: number, salt: number): number {
...
export function deal(params: BoardParams, seed: number): Tubes {
...
function acceptable(tubes: Tubes, params: BoardParams): boolean {
...
function findBoard(params: BoardParams, seedFor: (salt: number) => number, from = 0) {
...
export function levelBoard(n: number): LevelBoard {
...
export function seedBoard(seed: number): SeedBoard {
  const random = rng(seed)
  const params = {
