...
function el(tag: keyof HTMLElementTagNameMap, cls: string, html?: string) {
...
export function pickaxeSwing(x: number, side: number, boardWidth: number) {
...
function visitor(blockSvg: string) {
  return (
...
export function mine(boardEl: HTMLElement, trips: ActorTrip[], motion: Motion, hooks: { warp?: (rect: DOMRect) => void } = {}) {
  ...
  const rel = (r: DOMRect) => ({ x: r.left - board.left, y: r.top - board.top })

  const settled: Promise<Animation>[] = []
  ...
  const done = Promise.allSettled(settled).then(cancel)
  return { done, cancel }
}
