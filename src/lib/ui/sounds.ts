...
const visibilityChanged = () => { if (document.hidden) stopVoices(); else if (ctx) ac() }
const pointerDown = () => { if (ctx) ac() }
...
const MATERIALS: Record<string, (at: number, i: number) => void> = {
  glass(at, i) {
...
export const sound = {
  mount() {
    ...
    addEventListener('pointerdown', pointerDown, { passive: true, capture: true })
  },
  dispose() {
    if (mounted) {
      document.removeEventListener('visibilitychange', visibilityChanged)
      removeEventListener('pointerdown', pointerDown, { capture: true })
      mounted = false
    }
    stopVoices()
    const audio = ctx
    ctx = null
    noiseBuf = null
    return audio && audio.state !== 'closed' ? audio.close() : Promise.resolve()
  },
  ...
  reloadSettings() {
    muted = readSlot(KEY) === '1'
    if (muted) stopVoices()
  },
  toggle() {
    muted = !muted
    if (muted) stopVoices()
    writeSlot(KEY, muted ? '1' : '0')
    return muted
  },
}
