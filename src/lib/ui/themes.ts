...
export function themeByKey(key: string | null) {
  return SHELL_THEMES.find(t => t.key === key) ?? SHELL_THEMES[0]
}

export function loadTheme() {
  return themeByKey(readSlot(KEY))
}

export function saveTheme(key: string) {
  return writeSlot(KEY, key)
}
...
export function applyTheme(theme: ShellTheme) {
