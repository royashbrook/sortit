...
import { drawConfettiPiece } from './confetti-art.ts'

// the burst on screen, if any: a new board takes it down before its time
let active: { canvas: HTMLCanvasElement; frame: number } | null = null
...
export function confetti(colors: string[], skin = 'tubes') {
  clearConfetti()
  ...
  const frame = (now: number) => {
    ...
    for (const p of pieces) {
      ...
      if (p.stamp) g.drawImage(p.stamp, -12, -12, 24, 24)
      g.restore()
    }
